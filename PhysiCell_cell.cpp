...
Cell* custom_instantiate()
{
	std::cout << "jwb custom_instantiate was called" << std::endl;
        return new Cell;
}
...
Cell_Definition::Cell_Definition( Cell_Definition& cd )
{
	...
	// this is the whole reason we need to make a copy constructor 
	parameters.pReference_live_phenotype = &phenotype; 
...
//	{ functions.update_phenotype( this , phenotype , dt_ ); }
	...
	{
		//std::cout << "PhysiCell_cell line 331 - flagging cell for division:" << phenotype.flagged_for_division << " (Boolean)"  << std::endl;
		flag_for_division(); 
...
Cell* Cell::divide()
{
	...
	double child_flag = 0;	   //TODO - use the custom_data value for this rather than hard-coding it
	...
	for( int nn = 0 ; nn < custom_data.variables.size() ; nn++ )
	{
		...
		//uncomment the line below to print out the custom_data variables each turn
		//std::cout << "*** custom_data.variables[nn]: " << custom_data.variables[nn] << std::endl;

		if( custom_data.variables[nn].name == "cancer_bias" && custom_data.variables[nn].value > 0)
		{ 
			// calculate a random number * cancer_bias value to decide how to set child_flag
			mutation_chance = UniformRandom();
			std::cout << "- child cell cancer_bias is: " << custom_data.variables[nn].value << " - random check for mutation: " << mutation_chance << std::endl;
			if(mutation_chance <= custom_data.variables[nn].value)
			{ 
				std::cout << " --> child cell mutated to cancer" << std::endl;
				child_flag = 1;     // see below where child is created and this is used to change the type value to 1 (cancer cell)
			} else {
				std::cout << " --> child cell is normal stem cell" << std::endl;
				child_flag = 0;
			}
		}

		if (custom_data.variables[nn].name == "generation_count" && this->type == 1)
		{
			std::cout << "HERE we set custom_data.variables[nn].value += 1 " << this->type  << this->type_name << std::endl;
			custom_data.variables[nn].value += 1;
		}
	}
	...
	// jwb put a hook right here to operate on that fresh child cell
	//std::cout << "child cell hook for new child cell: " << child->ID << " time:" << PhysiCell_globals.current_time << std::endl;
	// end jwb changes
	//cancer_bias score resulted in a change of type for child cell
	if(child_flag == 1){
		child->type = 1;  //set to cancer type
		child->type_name = "cancer";     //TODO - they must have some function to completely convert the child over to a different cell type
	}
...
// #endif
	...
	// jwb added this to confirm child cell ID
	std::cout << "New child ID: " << child->ID << std::endl << std::endl;
	return child;
}
